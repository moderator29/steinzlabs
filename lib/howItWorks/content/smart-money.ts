...
'The leaderboard is now ranked by an independent Dune smart-money score (0 to 100), computed on-chain across Ethereum, BSC, Polygon, Arbitrum, Optimism, Base, and Avalanche, and each wallet is then enriched with our own profile for it, its name, archetype, verified badge, and recent moves, wherever we track it. The score, its win rate, and its profit-and-loss use a transparent ninety-day cash-flow basis: dollars received when a wallet sells a token minus dollars paid when it buys, with win rate being the share of token positions that came out net positive. This is a clear, auditable measure rather than exchange-style lot-by-lot accounting, so treat it as a directional edge signal, not a tax statement. When the Dune surface has not refreshed yet, the board falls back to our curated whale directory so it is never empty.',
  ],
  howToUse: [
