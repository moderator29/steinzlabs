 ...
 * under naka-labs.vercel.app while prod is nakalabs.xyz. We pin both
...
export function getRpConfig() {
  ...
  const origin = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://nakalabs.xyz';
  const url = (() => { try { return new URL(origin); } catch { return new URL('https://nakalabs.xyz'); } })();
