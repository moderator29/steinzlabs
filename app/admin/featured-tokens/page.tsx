...
import { Star, Plus, Trash2, GripVertical, Eye, EyeOff, Search } from 'lucide-react';
...
const MOCK: FeaturedToken[] = [
  { id: '1', symbol: 'PEPE',  name: 'Pepe',           chain: 'ETH',  address: '0x6982...8945', displayOrder: 1, active: true,  badge: 'Hot',      addedAt: Date.now() - 86400_000 },
  { id: '2', symbol: 'BONK',  name: 'Bonk',           chain: 'SOL',  address: 'DezX...GQ73',   displayOrder: 2, active: true,  badge: 'Trending', addedAt: Date.now() - 172800_000 },
  { id: '3', symbol: 'BRETT', name: 'Brett',          chain: 'BASE', address: '0x532f...A21C', displayOrder: 3, active: true,  badge: 'New',      addedAt: Date.now() - 3600_000 },
  { id: '4', symbol: 'WIF',   name: 'dogwifhat',      chain: 'SOL',  address: 'EKpy...3VxQ',   displayOrder: 4, active: true,  badge: '',         addedAt: Date.now() - 604800_000 },
  { id: '5', symbol: 'MOG',   name: 'Mog Coin',       chain: 'ETH',  address: '0xaae...E714', displayOrder: 5, active: false, badge: '',         addedAt: Date.now() - 259200_000 },
  { id: '6', symbol: 'FLOKI', name: 'FLOKI',          chain: 'BSC',  address: '0xCF0...B1C9', displayOrder: 6, active: true,  badge: 'Partner',  addedAt: Date.now() - 432000_000 },
];
...
export default function FeaturedTokensPage() {
  const [tokens, setTokens] = useState(MOCK);
  ...
  const moveUp = (id: string) => {
    ...
    setTokens(prev => prev.map(t => {
      if (t.id === id) return { ...t, displayOrder: t.displayOrder - 1 };
      if (t.displayOrder === tokens[idx].displayOrder - 1) return { ...t, displayOrder: t.displayOrder + 1 };
      return t;
    }));
  };
  ...
  const add = () => {
    ...
    const maxOrder = Math.max(0, ...tokens.map(t => t.displayOrder));
    setTokens(prev => [{ ...form, id: Date.now().toString(), displayOrder: maxOrder + 1, addedAt: Date.now() }, ...prev]);
    setForm({ ...BLANK });
    setShowForm(false);
  };
      ...
      {showForm && (
        <div className="bg-[#141824] border border-[#1E2433] rounded-xl p-4 mb-4 space-y-3">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={add} className="flex items-center gap-2 bg-[#0A1EFF] hover:bg-[#0818CC] text-white text-xs px-4 py-2 rounded-lg font-medium transition-colors">
              <Star className="w-3.5 h-3.5" /> Add Featured
            </button>
          </div>
        </div>
      )}
              ...
              <tr key={t.id} className="border-b border-[#1E2433] last:border-0 hover:bg-[#1E2433]/30">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-gray-400 font-mono w-4 text-center">{t.displayOrder}</span>
                  </div>
                </td>
                <td className="px-4 py-3">
