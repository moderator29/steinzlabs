...
import { Tag, Plus, Trash2, Edit2, Save, X, Search } from 'lucide-react';
...
const MOCK: WalletLabel[] = [
  { id: '1', address: '0xAb3f...B12E', chain: 'ETH', label: 'Binance Hot Wallet 1', category: 'Exchange', createdAt: Date.now() - 86400_000, verified: true },
  { id: '2', address: '9UeS...mK7J',  chain: 'SOL', label: 'Jump Trading',         category: 'Fund',     createdAt: Date.now() - 172800_000, verified: true },
  { id: '3', address: '0xF2a1...D94C', chain: 'BASE',label: 'Known Rug Artist',    category: 'Scammer',  createdAt: Date.now() - 3600_000,   verified: false, notes: 'Linked to 3 confirmed rug pulls' },
  { id: '4', address: '0x9B2d...A3E1', chain: 'ETH', label: 'Uniswap: Router v3',  category: 'Protocol', createdAt: Date.now() - 604800_000, verified: true },
];

const BLANK: Omit<WalletLabel, 'id' | 'createdAt'> = {
  address: '', chain: 'ETH', label: '', category: 'Other', notes: '', verified: false,
};

export default function WalletLabelsPage() {
  const [labels, setLabels] = useState(MOCK);
  ...
  const save = () => {
    ...
    if (editing) {
      ...
      setLabels(prev => [{ ...form, id: Date.now().toString(), createdAt: Date.now() }, ...prev]);
    }
    setForm({ ...BLANK });
    setShowForm(false);
  };
  ...
  const editLabel = (label: WalletLabel) => {
    setForm({ address: label.address, chain: label.chain, label: label.label, category: label.category, notes: label.notes ?? '', verified: label.verified });
    setEditing(label.id);
    setShowForm(true);
  };
      ...
      {showForm && (
        <div className="bg-[#141824] border border-[#1E2433] rounded-xl p-4 mb-4 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <input value={form.address} onChange={e => setForm(f => ({ ...f, address: e.target.value }))} placeholder="Wallet address"
            ...
            <input value={form.label} onChange={e => setForm(f => ({ ...f, label: e.target.value }))} placeholder="Entity label (e.g. Binance Hot Wallet)"
          ...
          <div className="flex items-center gap-4">
            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={form.verified} onChange={e => setForm(f => ({ ...f, verified: e.target.checked }))} className="accent-[#0A1EFF]" />
      ...
      <div className="bg-[#141824] border border-[#1E2433] rounded-xl overflow-hidden">
        <div className="p-3 border-b border-[#1E2433]">
          <div className="relative">
            ...
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search by address, label, or category..."
        ...
        <table className="w-full text-xs">
          <thead className="border-b border-[#1E2433]">
            <tr>{['Address', 'Chain', 'Label', 'Category', 'Verified', 'Notes', 'Actions'].map(h => (
          ...
          <tbody>
            {filtered.map(l => (
              <tr key={l.id} className="border-b border-[#1E2433] last:border-0 hover:bg-[#1E2433]/30">
                ...
                <td className="px-4 py-3"><span className="px-2 py-0.5 bg-[#1E2433] rounded text-[10px] font-mono text-gray-300">{l.chain}</span></td>
                ...
                <td className="px-4 py-3">
                  {l.verified ? <span className="text-green-400 text-[10px] font-semibold">Verified</span> : <span className="text-gray-600 text-[10px]">Unverified</span>}
                </td>
                <td className="px-4 py-3 text-gray-400 max-w-xs truncate">{l.notes ?? '—'}</td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => editLabel(l)} className="text-gray-500 hover:text-white transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => setLabels(prev => prev.filter(x => x.id !== l.id))} className="text-red-500/50 hover:text-red-400 transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </td>
              </tr>
            ))}
