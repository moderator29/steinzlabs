...
import { computeTradeRecap, periodRange } from '@/lib/coins/recap';

export const runtime = 'edge';

/**
 * Shareable "Naka Wrapped" card (PNG). URL shape:
 *   /api/coins/wrapped?user=<username>&period=month|year
 *
 * A recap of the user's real coin_trades over the last month (default) or year:
 * total volume traded, realized PnL, number of coins traded, best coin and win
 * rate, with a nakalabs.xyz + referral-code footer. Real numbers only: an empty
 * period gets a clean "no trades" card, a user who hid their holdings gets a
 * "private" card. Same deep-navy Naka look, size and runtime as position-card.
 */
...
export async function GET(request: NextRequest) {
  ...
  const period = param(request, 'period') === 'year' ? 'year' : 'month';
  const periodLabel = period === 'year' ? 'Past year' : 'Past month';
  ...
  if ((prof as { show_holdings?: boolean }).show_holdings === false) {
    return simpleCard('This wrapped is private', `@${handle}`);
  }

  const recap = await computeTradeRecap((prof as { id: string }).id, startISO, endISO);
  if (recap.tradeCount === 0) return simpleCard('No trades yet', `@${handle} · ${periodLabel}`);
  ...
  const stat = (labelText: string, value: string, color = '#fff') => (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6, width: 300 }}>
      ...
      <div style={{ fontSize: 40, fontWeight: 800, letterSpacing: -1, color, display: 'flex' }}>{value}</div>
    </div>
  );
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            <div style={{ fontSize: 48, fontWeight: 800, letterSpacing: -1, display: 'flex' }}>Naka Wrapped</div>
            <div style={{ fontSize: 24, color: MUTED, display: 'flex' }}>@{handle} · {periodLabel}</div>
          </div>
          <div style={{ fontSize: 20, letterSpacing: 4, color: MUTED, display: 'flex' }}>NAKA LABS</div>
        </div>

        {/* Stat grid */}
        <div style={{ marginTop: 44, display: 'flex', flexWrap: 'wrap', rowGap: 36, columnGap: 40 }}>
          {stat('Volume traded', fmtCompact(recap.volumeUsd))}
          {stat('Realized P&L', `${up ? '+' : '-'}${fmtUsd(Math.abs(recap.realizedUsd))}`, pnlColor)}
          {stat('Coins traded', String(recap.coinsTraded))}
