...
export default function WalletPage() {
  ...
  useEffect(() => {
    const stored = localStorage.getItem('naka_wallets');
    if (stored) {
      const parsed = JSON.parse(stored);
      setWallets(parsed);
      if (parsed.length > 0) setActiveWallet(parsed[0]);
    }
    const tokens = localStorage.getItem('naka_custom_tokens');
  ...
  const saveWallets = (w: StoredWallet[]) => {
    setWallets(w);
    localStorage.setItem('naka_wallets', JSON.stringify(w));
  };
  ...
  const fetchBalances = useCallback(async (address: string, chain: ChainInfo) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setWalletData(data);
        try {
          const existing = JSON.parse(localStorage.getItem('naka_portfolio_wallet') || '""');
          if (existing !== address) {
            localStorage.setItem('naka_portfolio_wallet', JSON.stringify(address));
            localStorage.setItem('wallet_address', address);
            localStorage.setItem('wallet_provider', 'builtin');
            window.dispatchEvent(new CustomEvent('naka_wallet_changed'));
          }
        } catch {}
      }
  ...
  if (view === 'add-token') return <AddTokenView onBack={() => setView('main')} tokens={customTokens} onAdd={(t) => { const updated = [...customTokens, t]; setCustomTokens(updated); localStorage.setItem('naka_custom_tokens', JSON.stringify(updated)); setView('main'); }} />;

  return (
    <div className="min-h-screen bg-[#0B0D14] text-white pb-24">
      <div className="max-w-lg mx-auto">
        {wallets.length === 0 ? (
          <div className="px-4 pt-8">
            <div className="text-center py-12">
              <div className="w-24 h-24 mx-auto mb-6 bg-gradient-to-br from-[#00D4AA] to-[#6366F1] rounded-3xl flex items-center justify-center shadow-2xl shadow-[#00D4AA]/20">
                <Wallet className="w-12 h-12 text-white" />
              </div>
              <h1 className="text-2xl font-heading font-bold mb-2">NAKA Wallet</h1>
              ...
              <div className="space-y-3">
                <button onClick={() => setView('create')} className="w-full py-4 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-2xl font-bold text-base flex items-center justify-center gap-2 shadow-lg shadow-[#00D4AA]/20">
              ...
              <div className="mt-6 p-4 bg-[#10B981]/5 rounded-2xl border border-[#10B981]/10">
                ...
                <p className="text-[11px] text-gray-500">Your keys stay on your device. NAKA never has access to your private keys or funds.</p>
              </div>
            </div>
          </div>
        ) : (
            ...
            <div className="px-4 pt-2 pb-4">
              ...
              <div className="flex justify-center gap-4 mb-6">
                {EVM_LIVE_CHAINS.includes(activeChain.id) ? (
                  <>
                    <ActionButton icon={<ArrowUpRight className="w-5 h-5" />} label="Send" color="#00D4AA" onClick={() => setView('send')} />
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {Object.keys(multiChainBalances).length > 0 && !hideBalance && (
                        <span className="text-xs font-mono text-[#00D4AA]">${totalMultiChainUsd.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                      )}
        ...
        {step === 'password' && (
          <div className="space-y-4">
            <div>
              ...
              <input value={walletName} onChange={e => setWalletName(e.target.value)} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#00D4AA]/50" />
            </div>
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block font-medium">Set Password (min 6 chars)</label>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="Secure password to encrypt your keys" />
            </div>
            ...
            <button onClick={createWallet} disabled={password.length < 6 || creating} className="w-full py-3.5 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-xl font-bold text-sm disabled:opacity-50">
        ...
        {step === 'phrase' && (
          <div className="space-y-4">
            ...
            <div className="p-3.5 bg-[#111827] rounded-xl border border-white/5">
              ...
              <p className="text-xs font-mono text-[#00D4AA] break-all">{address}</p>
            </div>

            <label className="flex items-center gap-2 cursor-pointer">
              <input type="checkbox" checked={confirmed} onChange={e => setConfirmed(e.target.checked)} className="rounded accent-[#00D4AA]" />
              <span className="text-xs text-gray-400">I have saved my recovery phrase securely</span>
            </label>

            <button onClick={confirmAndSave} disabled={!confirmed} className="w-full py-3.5 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-xl font-bold text-sm disabled:opacity-50 flex items-center justify-center gap-2">
    ...
    <div className="min-h-screen bg-[#0B0D14] text-white pb-24">
      <div className="px-4 pt-6 max-w-lg mx-auto">
        ...
        <div className="flex gap-2 mb-5 bg-[#111827] rounded-xl p-1">
          <button onClick={() => setMethod('phrase')} className={`flex-1 py-2.5 rounded-lg text-xs font-semibold transition-all ${method === 'phrase' ? 'bg-gradient-to-r from-[#00D4AA] to-[#6366F1] text-white' : 'text-gray-400'}`}>
            Recovery Phrase
          </button>
          <button onClick={() => setMethod('key')} className={`flex-1 py-2.5 rounded-lg text-xs font-semibold transition-all ${method === 'key' ? 'bg-gradient-to-r from-[#00D4AA] to-[#6366F1] text-white' : 'text-gray-400'}`}>
            Private Key
          </button>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <textarea value={input} onChange={e => setInput(e.target.value)}
              ...
              className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-sm font-mono focus:outline-none focus:border-[#00D4AA]/50 resize-none"
              placeholder={method === 'phrase' ? 'word1 word2 word3 ...' : '0x...'} />
          </div>
          <div>
            <label className="text-xs text-gray-400 mb-1.5 block font-medium">Set Password (min 6 chars)</label>
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="Encrypt your keys" />
          </div>
          {error && <p className="text-xs text-[#EF4444] bg-[#EF4444]/5 p-3 rounded-xl border border-[#EF4444]/10">{error}</p>}
          <button onClick={handleImport} disabled={importing || !input.trim() || password.length < 6} className="w-full py-3.5 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-xl font-bold text-sm disabled:opacity-50">
      ...
      <div className="px-4 pt-6 max-w-lg mx-auto">
        ...
        {status === 'success' ? (
          <div className="text-center py-8">
            ...
            <a href={`${chain.explorerUrl}/tx/${txHash}`} target="_blank" rel="noopener noreferrer" className="text-[#00D4AA] text-xs underline flex items-center justify-center gap-1">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input value={to} onChange={e => setTo(e.target.value)} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-sm font-mono focus:outline-none focus:border-[#00D4AA]/50" placeholder="0x..." />
            </div>
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block font-medium">Amount ({chain.symbol})</label>
              <input type="number" value={amount} onChange={e => setAmount(e.target.value)} step="0.001" className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-sm font-mono focus:outline-none focus:border-[#00D4AA]/50" placeholder="0.01" />
            </div>
            <div>
              <label className="text-xs text-gray-400 mb-1.5 block font-medium">Wallet Password</label>
              <input type="password" value={password} onChange={e => setPassword(e.target.value)} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="Enter your wallet password" />
            </div>
            {error && <p className="text-xs text-[#EF4444] bg-[#EF4444]/5 p-3 rounded-xl border border-[#EF4444]/10">{error}</p>}
            <button onClick={handleSend} disabled={status === 'sending' || !to || !amount || !password} className="w-full py-3.5 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-xl font-bold text-sm disabled:opacity-50">
      ...
      <div className="px-4 pt-6 max-w-lg mx-auto">
        ...
        <div className="text-center">
          ...
          <div className="bg-[#111827] border border-white/10 rounded-xl p-4 mb-4">
            <p className="text-xs font-mono break-all text-[#00D4AA]">{address}</p>
          </div>

          <button
            onClick={() => { navigator.clipboard.writeText(address); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
            className="w-full py-3.5 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-xl font-bold text-sm flex items-center justify-center gap-2">
      ...
      <div className="px-4 pt-6 max-w-lg mx-auto">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input value={address} onChange={e => { setAddress(e.target.value); setError(''); }} className="w-full bg-[#111827] border border-white/10 rounded-xl px-4 py-3 text-sm font-mono focus:outline-none focus:border-[#00D4AA]/50" placeholder="0x..." />
          </div>
          {error && <p className="text-xs text-[#EF4444]">{error}</p>}
          <button onClick={handleAdd} disabled={!address} className="w-full py-3.5 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-xl font-bold text-sm disabled:opacity-50">
            Add Token
          </button>
