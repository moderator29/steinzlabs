      ...
      <div className="px-4 pt-6">
        ...
        <div className="flex flex-col items-center mb-6">
          ...
          {walletConnected ? (
            <div className="flex items-center gap-2 text-xs text-gray-400">
              ...
              <button className="text-[#0A1EFF]"><Copy className="w-3 h-3" /></button>
            </div>
          ) : (
            <button className="bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] px-5 py-2 rounded-xl text-xs font-semibold hover:scale-105 transition-transform mt-1">
              Connect Wallet
            </button>
          )}
        </div>
        ...
        <div className="grid grid-cols-4 gap-2 mb-6">
          {[
            ...
            <div key={stat.label} className="glass rounded-lg p-2.5 text-center border border-white/10">
              <div className="text-sm font-bold bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] bg-clip-text text-transparent">{stat.value}</div>
        ...
        <div className="flex gap-1 mb-4 bg-[#111827] p-1 rounded-lg">
          {TABS.map((tab) => (
            <button
              ...
              className={`flex-1 py-2 rounded-md text-xs font-semibold transition-all ${activeTab === tab ? 'bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] text-white' : 'text-gray-400'}`}
            >
              {tab}
            </button>
          ))}
        ...
        {activeTab === 'Achievements' && (
          <div className="grid grid-cols-2 gap-2">
            {ACHIEVEMENTS.map((a) => (
              <div key={a.name} className={`glass rounded-xl p-3 border transition-all ${a.earned ? 'border-[#0A1EFF]/30 bg-[#0A1EFF]/5' : 'border-white/10 opacity-50'}`}>
