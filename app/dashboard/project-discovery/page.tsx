...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  DexScreener: 'bg-[#00D4AA]/20 text-[#00D4AA]',
        ...
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-1.5 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-lg text-[11px] font-bold flex items-center gap-1 hover:opacity-90 transition-opacity"
            >
            ...
            >
              <RotateCcw className={`w-4 h-4 text-[#00D4AA] transition-transform ${loading ? 'animate-spin' : 'group-hover:rotate-180 duration-500'}`} />
            </button>
          </div>
        </div>
          ...
          {CHAINS.map((c) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[10px] font-semibold whitespace-nowrap transition-colors ${
                activeChain === c.id
                  ? 'bg-gradient-to-r from-[#00D4AA] to-[#6366F1] text-white'
  ...
  return (
    <div className={`glass rounded-xl p-4 border transition-all hover:scale-[1.02] ${
      project.pinned
        ...
        : 'border-white/10 hover:border-[#00D4AA]/30'
    }`}>
      {project.pinned && (
      ...
      <a
        ...
        className="w-full py-1.5 rounded-lg border border-[#00D4AA]/30 text-[#00D4AA] text-[11px] font-semibold hover:bg-[#00D4AA]/10 transition-colors flex items-center justify-center gap-1"
      >
      ...
      <div className="bg-[#111827] border border-white/10 rounded-2xl max-w-lg w-full max-h-[85vh] overflow-y-auto p-6" onClick={e => e.stopPropagation()}>
        ...
        {submitted ? (
          <div className="text-center py-8">
            ...
            {previewUrl && (
              <a href={previewUrl} className="inline-flex items-center gap-2 px-4 py-2 bg-[#1A2235] border border-[#00D4AA]/30 rounded-lg text-xs text-[#00D4AA] hover:bg-[#00D4AA]/10 transition-colors mb-3">
            ...
            <button onClick={onClose} className="px-6 py-2 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-lg text-sm font-semibold">Close</button>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.tokenName} onChange={e => setForm({...form, tokenName: e.target.value})} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="NAKA GO" />
              </div>
              <div>
                <label className="text-[10px] text-gray-400 uppercase mb-1 block">Symbol *</label>
                <input value={form.symbol} onChange={e => setForm({...form, symbol: e.target.value})} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="NAKAGO" />
              </div>
            </div>
            <div>
              <label className="text-[10px] text-gray-400 uppercase mb-1 block">Contract Address *</label>
              <input value={form.contractAddress} onChange={e => setForm({...form, contractAddress: e.target.value})} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-[#00D4AA]/50" placeholder="0x..." />
            </div>
            <div>
              <label className="text-[10px] text-gray-400 uppercase mb-1 block">Chain *</label>
              <select value={form.chain} onChange={e => setForm({...form, chain: e.target.value})} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input value={form.website} onChange={e => setForm({...form, website: e.target.value})} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="https://" />
              </div>
              <div>
                ...
                <input value={form.telegram} onChange={e => setForm({...form, telegram: e.target.value})} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="@group" />
              </div>
              <div>
                <label className="text-[10px] text-gray-400 uppercase mb-1 block">Twitter</label>
                <input value={form.twitter} onChange={e => setForm({...form, twitter: e.target.value})} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="@handle" />
              </div>
            </div>
            <div>
              <label className="text-[10px] text-gray-400 uppercase mb-1 block">Description</label>
              <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})} rows={3} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50 resize-none" placeholder="Brief description of your token project..." />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[10px] text-gray-400 uppercase mb-1 block">Logo URL</label>
                <input value={form.logoUrl} onChange={e => setForm({...form, logoUrl: e.target.value})} className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="https://..." />
              </div>
              <div>
                <label className="text-[10px] text-gray-400 uppercase mb-1 block">Email *</label>
                <input value={form.email} onChange={e => setForm({...form, email: e.target.value})} type="email" className="w-full bg-[#0B0D14] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#00D4AA]/50" placeholder="team@project.com" />
              </div>
            </div>
            <button
              onClick={handleSubmit}
              disabled={submitting || !form.tokenName || !form.symbol || !form.contractAddress || !form.email}
              className="w-full py-2.5 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-lg text-sm font-bold disabled:opacity-50 hover:opacity-90 transition-opacity"
            >
