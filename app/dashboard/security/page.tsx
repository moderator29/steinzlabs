    ...
    <div className="min-h-screen bg-[#0A0E1A] text-white pb-20">
      <div className="sticky top-0 z-40 glass backdrop-blur-xl border-b border-white/10">
        <div className="flex items-center gap-3 px-4 h-14">
          ...
          <Shield className="w-5 h-5 text-[#0A1EFF]" />
          <h1 className="text-sm font-heading font-bold">Security Center</h1>
          <span className="ml-auto px-2 py-0.5 bg-[#0A1EFF]/20 text-[#0A1EFF] rounded text-[10px] font-semibold">GOPLUS</span>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-2">
          {CHAINS.map((chain) => (
            <button
              ...
              className={`rounded-xl py-2 px-3 border transition-all text-center text-xs font-semibold ${selectedChain === chain.key ? 'bg-[#0A1EFF]/10 border-[#0A1EFF]/30 text-[#0A1EFF]' : 'glass border-white/10 hover:border-white/20 text-gray-400'}`}
            >
              {chain.label}
            </button>
          ))}
