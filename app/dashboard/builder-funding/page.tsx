...
export default function BuilderFundingPage() {
  ...
  const milestoneIcon = (status: string) => {
    ...
    if (status === 'in_progress') return <Loader className="w-4 h-4 text-[#0A1EFF] animate-spin" />;
  ...
  const milestoneColor = (status: string) => {
    ...
    if (status === 'in_progress') return 'text-[#0A1EFF]';
    ...
    <div className="min-h-screen bg-[#0A0E1A] text-white pb-20">
      <div className="px-4 pt-6">
        ...
        <div className="grid grid-cols-4 gap-2 mb-4">
          ...
          <div className="glass rounded-xl p-2.5 border border-white/10 text-center">
            <div className="text-sm font-bold text-[#0A1EFF]">{stats.totalProjects}</div>
        ...
        <div className="flex gap-1 mb-4 bg-[#111827] p-1 rounded-lg">
          <button onClick={() => setActiveTab('browse')} className={`flex-1 py-2 rounded-md text-xs font-semibold transition-all ${activeTab === 'browse' ? 'bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] text-white' : 'text-gray-400'}`}>
            Browse Projects
          </button>
          <button onClick={() => setActiveTab('submit')} className={`flex-1 py-2 rounded-md text-xs font-semibold transition-all ${activeTab === 'submit' ? 'bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] text-white' : 'text-gray-400'}`}>
            Submit Project
          </button>
        </div>
        ...
        {activeTab === 'browse' && (
          <>
            {showInfo && (
              <div className="glass rounded-xl p-4 border border-[#0A1EFF]/20 mb-4 bg-gradient-to-r from-[#0A1EFF]/5 to-transparent">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <Info className="w-4 h-4 text-[#0A1EFF]" />
                ...
                <div className="space-y-3">
                  {STEPS.map((step, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <div className="w-6 h-6 rounded-full bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] flex items-center justify-center text-[10px] font-bold flex-shrink-0">{i + 1}</div>
                      <div>
            ...
            <div className="flex gap-2 mb-4 overflow-x-auto scrollbar-hide">
              {['All', 'Active', 'Funded', 'DeFi', 'Security', 'Infrastructure', 'Governance'].map((f) => (
                <button key={f} onClick={() => setActiveFilter(f)} className={`px-3 py-1.5 rounded-lg text-[11px] font-semibold whitespace-nowrap transition-colors ${activeFilter === f ? 'bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] text-white' : 'bg-[#111827] text-gray-400'}`}>
                  {f}
                </button>
              ))}
            </div>
                ...
                {filteredProjects.map((project) => {
                  ...
                  return (
                    <div key={project.id} className="glass rounded-xl border border-white/10 overflow-hidden">
                      <div className="h-24 bg-gradient-to-br from-[#0A1EFF]/10 via-[#7C3AED]/10 to-[#0A0E1A] flex items-center justify-between px-4">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${project.status === 'active' ? 'bg-[#10B981]/20 text-[#10B981]' : project.status === 'funded' ? 'bg-[#0A1EFF]/20 text-[#0A1EFF]' : 'bg-[#F59E0B]/20 text-[#F59E0B]'}`}>
                      ...
                      <div className="p-4">
                        ...
                        <div className="mb-3">
                          ...
                          <div className="w-full bg-[#111827] rounded-full h-2.5">
                            <div className="bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] h-2.5 rounded-full transition-all" style={{ width: `${Math.min(pct, 100)}%` }}></div>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          ...
                          <span className="px-2 py-0.5 bg-[#0A1EFF]/10 text-[#0A1EFF] rounded text-[9px] font-semibold flex items-center gap-1">
                        ...
                        <div className="flex gap-2">
                          ...
                          {project.status === 'active' && (
                            <button onClick={() => setFundingModal(project)} className="flex-1 py-2 rounded-lg bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] text-xs font-semibold hover:scale-105 transition-transform flex items-center justify-center gap-1">
        ...
        {activeTab === 'submit' && (
          <div className="space-y-3">
            <div className="glass rounded-xl p-4 border border-[#0A1EFF]/20 bg-gradient-to-r from-[#0A1EFF]/5 to-transparent mb-2">
            ...
            <div className="glass rounded-xl p-4 border border-white/10">
              ...
              <input value={form.name} onChange={e => setForm(p => ({...p, name: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#0A1EFF]/50" placeholder="Enter project name" />
            </div>

            <div className="glass rounded-xl p-4 border border-white/10">
              <label className="text-[10px] text-gray-500 mb-1 block">Description *</label>
              <textarea value={form.description} onChange={e => setForm(p => ({...p, description: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#0A1EFF]/50 min-h-[80px]" placeholder="Describe your project, problem it solves, and technical approach..." />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="glass rounded-xl p-4 border border-white/10">
                <label className="text-[10px] text-gray-500 mb-1 block">Category</label>
                <select value={form.category} onChange={e => setForm(p => ({...p, category: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#0A1EFF]/50">
              ...
              <div className="glass rounded-xl p-4 border border-white/10">
                ...
                <select value={form.chain} onChange={e => setForm(p => ({...p, chain: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#0A1EFF]/50">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="glass rounded-xl p-4 border border-white/10">
                ...
                <input type="number" value={form.goal} onChange={e => setForm(p => ({...p, goal: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#0A1EFF]/50" placeholder="100000" />
              </div>
              <div className="glass rounded-xl p-4 border border-white/10">
                <label className="text-[10px] text-gray-500 mb-1 block">Team Size</label>
                <input type="number" value={form.teamSize} onChange={e => setForm(p => ({...p, teamSize: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-[#0A1EFF]/50" placeholder="3" />
              </div>
            </div>

            <div className="glass rounded-xl p-4 border border-white/10">
              <label className="text-[10px] text-gray-500 mb-1 block">Your Name / Team Name</label>
              <input value={form.builderName} onChange={e => setForm(p => ({...p, builderName: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#0A1EFF]/50" placeholder="Builder or team name" />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="glass rounded-xl p-4 border border-white/10">
                <label className="text-[10px] text-gray-500 mb-1 block">Website</label>
                <input value={form.website} onChange={e => setForm(p => ({...p, website: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#0A1EFF]/50" placeholder="https://..." />
              </div>
              <div className="glass rounded-xl p-4 border border-white/10">
                <label className="text-[10px] text-gray-500 mb-1 block">Whitepaper</label>
                <input value={form.whitepaper} onChange={e => setForm(p => ({...p, whitepaper: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#0A1EFF]/50" placeholder="https://..." />
              </div>
            </div>

            <div className="glass rounded-xl p-4 border border-white/10">
              <label className="text-[10px] text-gray-500 mb-1 block">Tags (comma separated)</label>
              <input value={form.tags} onChange={e => setForm(p => ({...p, tags: e.target.value}))} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2 text-xs focus:outline-none focus:border-[#0A1EFF]/50" placeholder="DeFi, Cross-chain, MEV Protection" />
            </div>

            <div className="glass rounded-xl p-4 border border-white/10">
              <h3 className="text-xs font-bold mb-3 text-[#0A1EFF]">Milestone Plan</h3>
              {form.milestones.map((m, i) => (
                <div key={i} className="mb-3 pb-3 border-b border-white/5 last:border-0 last:mb-0 last:pb-0">
                  ...
                  <div className="space-y-1.5">
                    ...
                    }} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-[#0A1EFF]/50" placeholder="Milestone name" />
                    <textarea value={m.description} onChange={e => {
                      const ms = [...form.milestones];
                      ms[i] = {...ms[i], description: e.target.value};
                      setForm(p => ({...p, milestones: ms}));
                    }} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-[#0A1EFF]/50 min-h-[40px]" placeholder="Description & deliverables" />
                    <input type="number" value={m.amount} onChange={e => {
                      const ms = [...form.milestones];
                      ms[i] = {...ms[i], amount: e.target.value};
                      setForm(p => ({...p, milestones: ms}));
                    }} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-1.5 text-xs focus:outline-none focus:border-[#0A1EFF]/50" placeholder="Amount ($)" />
                  </div>
                </div>
              ))}
            ...
            <button onClick={handleSubmitProject} disabled={submitting || !form.name || !form.description || !form.goal} className="w-full py-3 rounded-xl bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] font-semibold text-sm hover:scale-105 transition-transform disabled:opacity-50 flex items-center justify-center gap-2">
          ...
          <div className="w-full max-w-sm bg-[#0A0E1A] rounded-2xl border border-white/10 p-5" onClick={e => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="text-[#0A1EFF]">{Math.round((fundingModal.raised / fundingModal.goal) * 100)}%</span>
              </div>
              <div className="w-full bg-[#111827] rounded-full h-2">
                <div className="bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] h-2 rounded-full" style={{ width: `${Math.min((fundingModal.raised / fundingModal.goal) * 100, 100)}%` }}></div>
              </div>
            ...
            <div className="mb-4">
              ...
              <input type="number" value={fundAmount} onChange={e => setFundAmount(e.target.value)} className="w-full bg-[#111827] border border-white/10 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:border-[#0A1EFF]/50" placeholder="1000" />
            ...
            <button onClick={handleFund} disabled={funding || !fundAmount || parseFloat(fundAmount) <= 0} className="w-full py-3 rounded-xl bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] font-semibold text-sm disabled:opacity-50 flex items-center justify-center gap-2">
