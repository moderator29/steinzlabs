      ...
      <div className="p-4 space-y-4 max-w-md mx-auto">
        ...
        <div className="flex justify-center -my-2 relative z-10">
          <button className="w-10 h-10 bg-[#111827] border border-white/10 rounded-xl flex items-center justify-center hover:bg-white/10 transition-colors hover:rotate-180 duration-300">
            <ArrowLeftRight className="w-4 h-4 text-[#0A1EFF]" />
          </button>
        </div>
        ...
        {fromAmount && (
          <div className="glass rounded-xl p-3 border border-white/10 space-y-2">
            ...
            <div className="flex justify-between text-[10px]">
              ...
              <span className="text-[#0A1EFF]">Uniswap V3 (Best)</span>
            </div>
          </div>
        )}

        <button className="w-full bg-gradient-to-r from-[#0A1EFF] to-[#7C3AED] py-4 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 hover:scale-[1.02] transition-transform">
        ...
        <div className="flex gap-2 justify-center">
          {['0.1%', '0.5%', '1.0%', '3.0%'].map((s) => (
            <button key={s} onClick={() => setSlippage(s.replace('%', ''))} className={`px-3 py-1.5 rounded-lg text-[10px] font-semibold ${slippage === s.replace('%', '') ? 'bg-[#0A1EFF]/20 text-[#0A1EFF]' : 'bg-[#111827] text-gray-500'}`}>
              {s}
            </button>
          ))}
        </div>
