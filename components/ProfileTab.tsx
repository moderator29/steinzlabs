...
export default function ProfileTab() {
    ...
    async function loadNotifications() {
      try {
        ...
        if (!cancelled && Array.isArray(data.notifications)) {
          const readIds = JSON.parse(localStorage.getItem('naka_read_notifs') || '[]');
  ...
  const markAsRead = (id: string) => {
    setNotifList(prev => {
      ...
      localStorage.setItem('naka_read_notifs', JSON.stringify(readIds));
      return updated;
    });
  };
  ...
  const getNotifIcon = (type: Notification['type']) => {
    switch (type) {
      case 'whale': return <AlertTriangle className="w-4 h-4 text-[#00D4AA]" />;
  ...
  useEffect(() => {
    const saved = localStorage.getItem('naka_support_chat');
    if (saved) {
  ...
  useEffect(() => {
    if (chatMessages.length > 0) {
      localStorage.setItem('naka_support_chat', JSON.stringify(chatMessages));
    }
  }, [chatMessages]);
  ...
  const clearChat = () => {
    setChatMessages([]);
    localStorage.removeItem('naka_support_chat');
  };
  ...
  useEffect(() => {
    const savedNotifs = localStorage.getItem('naka_notifications');
    if (savedNotifs) {
      try { setNotifications(JSON.parse(savedNotifs)); } catch {}
    }
    const savedPrivacy = localStorage.getItem('naka_privacy');
    if (savedPrivacy) {
      try { setPrivacySettings(JSON.parse(savedPrivacy)); } catch {}
    }
    const savedPrefs = localStorage.getItem('naka_preferences');
    if (savedPrefs) {
  ...
  useEffect(() => {
    localStorage.setItem('naka_notifications', JSON.stringify(notifications));
  }, [notifications]);

  useEffect(() => {
    localStorage.setItem('naka_privacy', JSON.stringify(privacySettings));
  }, [privacySettings]);

  useEffect(() => {
    localStorage.setItem('naka_preferences', JSON.stringify(preferences));
  }, [preferences]);
          ...
          {Object.entries(privacySettings).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between px-3 py-3 border-b border-white/5 last:border-0">
              <div className="flex items-center gap-3">
                {key === 'showWallet' ? <Wallet className="w-4 h-4 text-[#6366F1]" /> :
                 key === 'showActivity' ? <BarChart3 className="w-4 h-4 text-[#00D4AA]" /> :
              ...
              <button
                ...
                className={`w-10 h-5 rounded-full transition-colors relative ${value ? 'bg-[#00D4AA]' : 'bg-gray-600'}`}
              >
  ...
  if (subPage === 'help') {
    const faqs = [
      ...
      { q: 'What is a Trust Score?', a: 'Trust Score is a 0-100 rating NAKA assigns to every on-chain event, token, and wallet. It considers contract verification, liquidity, holder distribution, developer history, and more.' },
      ...
      { q: 'Is my wallet data safe?', a: 'Yes. NAKA is 100% non-custodial. We only READ public blockchain data. Your private keys never leave your wallet. We cannot move your funds.' },
    ...
    return (
      <div>
        ...
        <div className="mt-6 glass rounded-lg p-4 border border-white/10 text-center">
          ...
          <a href="mailto:support@nakalabs.com" className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-lg text-xs font-semibold hover:scale-105 transition-transform">
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-heading font-bold flex items-center gap-2">
              <Bot className="w-5 h-5 text-[#00D4AA]" /> AI Support
            </h2>
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            {chatMessages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center py-8">
                <div className="w-12 h-12 bg-gradient-to-br from-[#00D4AA]/20 to-[#6366F1]/20 rounded-full flex items-center justify-center mb-3 border border-white/10">
                ...
                <div className="flex flex-wrap gap-2 justify-center">
                  {['How do I connect my wallet?', 'Is my data safe?', 'How do predictions work?'].map(q => (
                    <button
                      ...
                      className="text-[10px] px-3 py-1.5 rounded-full border border-white/10 text-gray-400 hover:text-white hover:border-[#00D4AA]/30 transition-colors"
                    >
                      {q}
                    </button>
                  ))}
            ...
            {chatMessages.map((msg, i) => (
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] rounded-xl px-3 py-2 ${msg.role === 'user' ? 'bg-gradient-to-r from-[#00D4AA]/20 to-[#6366F1]/20 border border-[#00D4AA]/20' : 'bg-white/5 border border-white/10'}`}>
                  {msg.role === 'assistant' && (
                    <div className="flex items-center gap-1.5 mb-1">
                      <Bot className="w-3 h-3 text-[#00D4AA]" />
                      <span className="text-[9px] text-[#00D4AA] font-semibold">NAKA Support</span>
                    </div>
                  )}
          ...
          <div className="border-t border-white/10 p-2">
            <div className="flex items-center gap-2">
              <input
                ...
                className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-[#00D4AA]/30"
              />
              <button
                onClick={sendSupportMessage}
                disabled={!chatInput.trim() || chatLoading}
                className="p-2 bg-gradient-to-r from-[#00D4AA] to-[#6366F1] rounded-lg hover:scale-105 transition-transform disabled:opacity-40 disabled:hover:scale-100"
              >
  ...
  if (subPage === 'preferences') {
    return (
      <div>
        ...
        <p className="text-xs text-gray-500 mb-4">Customize your NAKA experience.</p>
        ...
        <div className="glass rounded-lg border border-white/10 overflow-hidden mb-3">
          <div className="flex items-center justify-between px-3 py-3 border-b border-white/5">
            ...
            <select
              ...
              className="bg-[#111827] border border-white/10 rounded-lg px-2 py-1 text-xs text-gray-300 focus:outline-none focus:border-[#00D4AA]/30"
            >
          ...
          {[
            ...
            <div key={item.key} className="flex items-center justify-between px-3 py-3 border-b border-white/5 last:border-0">
              ...
              <button
                ...
                className={`w-10 h-5 rounded-full transition-colors relative ${(preferences as any)[item.key] ? 'bg-[#00D4AA]' : 'bg-gray-600'}`}
              >
  ...
  return (
    <div>
          ...
          <div className="border-t border-white/5">
            {unreadCount > 0 && (
              <div className="flex justify-end px-4 py-2 border-b border-white/5">
                <button onClick={markAllRead} className="text-[10px] text-[#00D4AA] hover:text-[#00D4AA]/80 font-semibold transition-colors">
                  Mark all read
                </button>
              </div>
            )}
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!n.read && <span className="w-1.5 h-1.5 bg-[#00D4AA] rounded-full flex-shrink-0" />}
                      </div>
      ...
      <div className="flex flex-col items-center mb-6">
        <div className="w-20 h-20 bg-gradient-to-br from-[#00D4AA]/20 to-[#6366F1]/20 rounded-full flex items-center justify-center mb-3 border-2 border-white/10">
          {user?.user_metadata?.avatar_url ? (
            <img src={user.user_metadata.avatar_url} alt="" className="w-full h-full rounded-full object-cover" />
          ) : (
            <User className="w-10 h-10 text-[#00D4AA]" />
          )}
        </div>
      ...
      {walletAddress && (
        ...
        >
          <ExternalLink className="w-4 h-4 text-[#00D4AA]" />
...
function HelpItem({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div className={`glass rounded-xl overflow-hidden border transition-all duration-300 ${open ? 'border-[#00D4AA]/30 bg-gradient-to-r from-[#00D4AA]/[0.03] to-[#6366F1]/[0.03]' : 'border-white/[0.06]'}`}>
      <button onClick={() => setOpen(!open)} className="w-full px-4 py-3 flex justify-between items-center text-left hover:bg-white/[0.02] transition-colors">
        ...
        <ChevronDown className={`w-4 h-4 text-[#00D4AA] transition-transform duration-300 flex-shrink-0 ${open ? 'rotate-180' : ''}`} />
      </button>
