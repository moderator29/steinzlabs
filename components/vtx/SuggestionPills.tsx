    ...
    <div className="flex flex-wrap gap-2 mt-3">
      {suggestions.slice(0, 4).map((s, i) => (
        <button
          key={`${s}-${i}`}
          onClick={() => onPick(s)}
          className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-white/[0.03] border border-white/10 hover:border-[#0A1EFF]/40 hover:bg-[#0A1EFF]/[0.08] text-[12px] text-slate-200 hover:text-white transition-all"
          style={{
