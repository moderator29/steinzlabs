    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={fetchProjects} disabled={loading} className="p-1.5 hover:bg-white/10 rounded-lg transition-all group">
          <RotateCcw className={`w-3.5 h-3.5 text-[#00D4AA] ${loading ? 'animate-spin' : 'group-hover:rotate-180 transition-transform duration-500'}`} />
        </button>
      </div>
      ...
      {loading && projects.length === 0 ? (
        <div className="text-center py-10">
          <RotateCcw className="w-6 h-6 text-[#00D4AA] mx-auto mb-2 animate-spin" />
          ...
          {filtered.map((project, i) => {
            ...
            return (
              <div key={`${project.slug}-${i}`} className={`glass rounded-xl p-3 border transition-all ${
                project.pinned ? 'border-[#FFD700]/30 bg-gradient-to-r from-[#FFD700]/5 to-transparent' : 'border-white/10 hover:border-[#00D4AA]/20'
              }`}>
      ...
      <div className="flex gap-2 mt-3">
        <button
          ...
          className="flex-1 py-2 rounded-lg border border-white/10 text-xs font-semibold text-[#00D4AA] hover:bg-white/5 transition-colors flex items-center justify-center gap-1"
        >
          View All <ExternalLink className="w-3 h-3" />
        </button>
        <button
          onClick={() => router.push('/dashboard/project-discovery')}
          className="px-4 py-2 rounded-lg bg-gradient-to-r from-[#00D4AA] to-[#6366F1] text-xs font-semibold flex items-center gap-1"
        >
